...
input_json_file = r"C:\Users\lukas\Documents\programowanie_gis\projekt\bus_data.txt"
...
# Sprawdzenie, czy pola już istnieją, jeśli nie - dodanie ich do warstwy
existing_fields = [f.name for f in arcpy.ListFields(output_shapefile)]
for field_name, field_type in fields:
    if field_name not in existing_fields:
        arcpy.AddField_management(output_shapefile, field_name, field_type)

# Wczytanie danych o autobusach z pliku JSON
bus_data = read_bus_data(input_json_file)

# Definiowanie układów współrzędnych: źródłowego (WGS84) i docelowego (CS92)
spatial_ref_wgs84 = arcpy.SpatialReference(4326)
spatial_ref_cs92 = arcpy.SpatialReference(2180)

# Słownik do sumowania opóźnień dla poszczególnych linii autobusowych
route_delays = {}
